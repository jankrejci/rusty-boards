...
/// 4028. The HTTP variant sends GET requests to a firmware-specific REST
/// API path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Endpoint {
    /// Cgminer TCP API command (e.g. "stats", "summary").
    Cgminer(&'static str, ScrapeTier),
    /// HTTP REST API path (e.g. "/api/v1/status").
    #[allow(dead_code)]
    Http(&'static str, ScrapeTier),
}
...
impl Endpoint {
    ...
    pub fn command(self) -> &'static str {
        match self {
            Self::Cgminer(cmd, _) | Self::Http(cmd, _) => cmd,
        }
    }
    ...
    pub fn tier(self) -> ScrapeTier {
        match self {
            Self::Cgminer(_, tier) | Self::Http(_, tier) => tier,
        }
    }
    ...
    /// For HTTP endpoints, sends a GET request to the path.
    pub async fn fetch(self, host: &str) -> Result<Value> {
        match self {
            Self::Cgminer(cmd, _) => cgminer_command(host, cmd).await,
            Self::Http(_, _) => anyhow::bail!("HTTP endpoints not yet supported"),
        }
    }
}
...
impl Firmware {
    ...
    pub fn identify(stats: &Value) -> Firmware {
...
/// Check whether a cgminer response indicates an error.
///
/// Inspects the STATUS array for an "E" (error) status code.
pub(crate) fn is_error(response: &Value) -> bool {
    response
        .get("STATUS")
...
async fn cgminer_command(host: &str, cmd: &str) -> Result<Value> {
